...
const EventPage = () => {
    ...
    return (
        <>
            {event && <Seo title={`${event.title} - ${event.subtitle}`} description={`Relic presents ${event.title} from ${performanceDateExtractor(event.performances)} in ${performanceLocationExtractor(event.performances)}`} />}
            <Box mb={5}>
                {/* Banner */}
                {eventBanner ? <Box
                    sx={{
